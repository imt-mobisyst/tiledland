...
from src.tiledland import Entity, Tile, draw
...
def test_fast_tile_init():
    ...
    assert tile.projectedShape().asZipped() == [(-0.5, -0.5), (-0.5, 0.5), (0.5, 0.5), (0.5, -0.5)]
    ...
    assert tile.projectedShape().asZipped() == [(-10.7, -11.3), (-10.7, 30.7), (31.3, 30.7), (31.3, -11.3)]
    assert tile.adjacencies() == []
    assert tile.entities() == []
    ...
    assert tile.projectedShape().asZipped() == [(0.0, 0.0), (0.0, 2.0), (2.0, 2.0), (2.0, 0.0)]
    assert tile.adjacencies() == []
    assert tile.entities() == []
...
def test_fast_tile_regular():
    ...
    assert tile.projectedShape().size() == 6
    limits= [ ( round(x, 2), round(y, 2) ) for x, y in tile.projectedShape().asZipped() ]
    print( limits )
    assert limits == [
...
def test_fast_tile_str():
    ...
    assert tile.position().asTuple() == (0.0, 0.0)
    assert tile.projectedShape().asZipped() == []
...
def test_fast_tile_absobj():
    ...
    assert tree.digits() == [ tile.index(), tile.group() ] + [ 1, 3, 7, 19 ]
    ...
    assert dt.digits() == [ tile.index(), tile.group() ] + [ 1, 3, 7, 19 ]
