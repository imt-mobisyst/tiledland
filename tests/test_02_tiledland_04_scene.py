...
def test_Scene_initLine():
    ...
    assert scene.tile(1).body().asZipped() == [(-0.5, -0.5), (-0.5, 0.5), (0.5, 0.5), (0.5, -0.5)]

    assert scene.tile(2).position().asTuple() == (1.1, 0.0)
    env= [ (round(x, 2), round(y, 2)) for x, y in scene.tile(2).body().asZipped() ]
    assert env == [(0.6, -0.5), (0.6, 0.5), (1.6, 0.5), (1.6, -0.5)]

    assert scene.tile(3).position().asTuple() == (2.2, 0.0)
    env= [ (round(x, 2), round(y, 2)) for x, y in scene.tile(3).body().asZipped() ]
...
def test_Scene_box():
    ...
    scene= Scene().initializeLine(4, connect=False)
    print( scene.box() )
    assert str(scene.box()) == "⌊(-0.5, -0.5), (3.8, 0.5)⌉"
